...
const News =(props)=> {
  ...
  const [totalResults,setTotalResults]= useState(0)
   ...
   const updateNews=async()=> {
    ...
    const url= `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=741f2f4961574aebaffadba0e4e06911&page=${page}&pageSize=${props.pageSize}`;
    setLoading(true)
  ...
  const fetchMoreData = async() => {
     const url= `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=741f2f4961574aebaffadba0e4e06911&page=${page+1}&pageSize=${props.pageSize}`;
     setPage(page+1)
     let data = await fetch(url);
     let parsedData = await data.json()
     console.log(parsedData); 
